...
<head>
    ...
    <title>{{ config('app.name') }} – @if(request()->routeIs('register')){{ __('messages.Register') }}@elseif(request()->routeIs('password.*')){{ __('Forgot your password?') }}@else{{ __('messages.Login') }}@endif</title>
...
<body class="body-texture text-etno-dark min-h-screen antialiased">
    ...
    <header x-data="{ mobileMenuOpen: false }" class="bg-etno-cream/90 backdrop-blur sticky top-0 z-50 border-b border-etno-clay/30">
        <div class="site-container flex items-center justify-between h-14 gap-8">
            ...
            <div class="md:hidden flex items-center gap-2">
                <livewire:locale-switcher />
                <button @click="mobileMenuOpen = ! mobileMenuOpen" type="button" class="p-2.5 rounded-4 text-etno-brown hover:bg-etno-sand hover:text-etno-terracotta focus:outline-none focus:ring-2 focus:ring-etno-clay" aria-label="{{ __('messages.Menu') }}">
        ...
        <div x-show="mobileMenuOpen" x-cloak
            ...
            <nav class="site-container py-4 flex flex-col gap-1 text-center">
                <a href="{{ route('home') }}" class="py-3 text-etno-dark font-medium text-lg border-b border-etno-clay/30 hover:bg-etno-sand/50 rounded-4">{{ __('messages.Home') }}</a>
                <a href="{{ route('products.index') }}" class="py-3 text-etno-dark font-medium text-lg border-b border-etno-clay/30 hover:bg-etno-sand/50 rounded-4">{{ __('messages.Products') }}</a>
                <a href="{{ route('cart.index') }}" class="py-3 text-etno-dark font-medium text-lg border-b border-etno-clay/30 hover:bg-etno-sand/50 rounded-4 flex items-center justify-center gap-2">
                    {{ __('messages.Cart') }}
                    @if(isset($cartCount) && $cartCount > 0)
                ...
                <a href="{{ route('login') }}" class="py-3 text-etno-dark font-medium text-lg border-b border-etno-clay/30 hover:bg-etno-sand/50 rounded-4">{{ __('messages.Login') }}</a>
                <a href="{{ route('register') }}" class="py-3 text-etno-dark font-medium text-lg border-b border-etno-clay/30 hover:bg-etno-sand/50 rounded-4">{{ __('messages.Register') }}</a>
    ...
    <main class="site-container py-8 md:py-12">
        <div class="min-h-[50vh] flex flex-col sm:justify-center items-center">
            <div class="w-full sm:max-w-md px-6 py-6 bg-white kafana-panel rounded-4 shadow-kafana overflow-hidden">
                {{ $slot }}
            </div>
        </div>
    </main>

    <footer class="border-t border-etno-clay/30 py-10 bg-etno-beige/60 mt-auto">
